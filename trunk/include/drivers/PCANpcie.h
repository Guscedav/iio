...
/**
 * This class implements a specific CAN device driver for the Peak System PCAN-PCIexpress mini interface board.
 * The PCAN-PCIexpress mini board features two SJA1000 CAN bus controllers that allow to transmit and receive
 * CAN 2.0B compliant frames. They offer a receive buffer with a size of 64 bytes.
 * <div style="text-align:center"><img src="pcanpcie.png" width="500"/></div>
 * <div style="text-align:center"><b>The Peak PCAN-PCIexpress mini CAN interface board</b></div>
 */
class PCANpcie : public CAN, public RealtimeThread {

    public:

                        PCANpcie(PCI& pci, uint16_t number, uint16_t port);
        virtual         ~PCANpcie();
    ...
    private:
        ...
        static const uint16_t   DEVICE_ID = 0x0008;     // device ID of this board
