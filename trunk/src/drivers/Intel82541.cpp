...
#include "PCI.h"
#include "Thread.h"
#include "Intel82541.h"

using namespace std;

const uint8_t Intel82541::MAC_ADDRESS[] = {0x00, 0x1b, 0x21, 0xd2, 0xd2, 0x70}; // ethernet address to use

/**
 * Creates a device driver object for the Intel 82541 Ethernet controller.
 ...
 */
Intel82541::Intel82541(PCI& pci, uint16_t number, uint32_t speed, bool loopback) : pci(pci) {
    
    // get base address
    
    deviceHandle = pci.openDevice(VENDOR_ID, DEVICE_ID, number);
    baseAddress = pci.getBaseAddress(1, VENDOR_ID, DEVICE_ID, number);
    
    if (baseAddress == 0) throw invalid_argument("Intel82541: this board was not found on the PCI bus.");
    
    // initialize the Intel 82541 Ethernet controller
    
    uint32_t ctrl = pci.in32(baseAddress+CTRL);
    pci.out32(baseAddress+CTRL, ctrl | 0x04000000);   // reset the Intel 82541
    ...
    pci.out32(baseAddress+RDBAL, receiveBufferPhysicalAddress & 0xFFFFFFFF);
    pci.out32(baseAddress+RDBAH, 0x00000000);
    pci.out32(baseAddress+RDLEN, DESCRIPTOR_LENGTH*16);
    pci.out32(baseAddress+RDH, 0x00000000);
    pci.out32(baseAddress+RDT, DESCRIPTOR_LENGTH-1);
    ...
    if (speed == 10) pci.out32(baseAddress+CTRL, (pci.in32(baseAddress+CTRL) & (~0x00001B61)) | 0x00001841);
    else if (speed == 100) pci.out32(baseAddress+CTRL, (pci.in32(baseAddress+CTRL) & (~0x00001B61)) | 0x00001941);
    else if (speed == 1000) pci.out32(baseAddress+CTRL, (pci.in32(baseAddress+CTRL) & (~0x00001B61)) | 0x00001A41);
    else pci.out32(baseAddress+CTRL, (pci.in32(baseAddress+CTRL) & (~0x00001B61)) | 0x00001361);
    
    uint32_t counter = 0;
    while (((pci.in32(baseAddress+STATUS) & 0x00000002) == 0) && (counter++ < LINK_UP_TIMEOUT)) Thread::sleep(1);
    
    if ((pci.in32(baseAddress+STATUS) & 0x00000002) == 0) cerr << "Intel82541: link not up!" << endl;
}

/**
 * Deletes the device driver object for the Intel 82541 Ethernet controller and releases all allocated resources.
 */
Intel82541::~Intel82541() {
 ...
 */
void Intel82541::address(uint8_t macAddress[6]) {
 ...
 */
uint16_t Intel82541::send(uint8_t destinationMACAddress[6], uint16_t etherType, uint8_t data[], uint16_t length) {
    
    uint32_t head = pci.in32(baseAddress+TDH);
    uint32_t tail = pci.in32(baseAddress+TDT);
    
    if (head == ((tail == DESCRIPTOR_LENGTH-1) ? 0 : tail+1)) throw overflow_error("Intel82541: the transmit buffer is full.");
 ...
 */
uint16_t Intel82541::receive(uint8_t sourceMACAddress[6], uint16_t& etherType, uint8_t data[], uint16_t length) {
    
    uint32_t head = pci.in32(baseAddress+RDH);
    uint32_t tail = pci.in32(baseAddress+RDT); tail = (tail == DESCRIPTOR_LENGTH-1) ? 0 : tail+1;
    uint8_t status = in8(receiveBufferAddress+12+tail*16);
    
    if ((tail != head) && ((status & 0x1) > 0)) {
        
        out8(receiveBufferAddress+12+tail*16, 0x00);    // reset status byte in descriptor
        pci.out32(baseAddress+RDT, tail);               // move tail pointer ahead
 ...
 */
void Intel82541::allocateDMA(uint32_t size, uint64_t& address, uint64_t& physicalAddress) {
    ...
    if (error != 0) {
        cerr << "Intel82541: mem_offset=" << error << " errno=" << errno << endl;
 ...
 */
void Intel82541::deallocateDMA(uint64_t address) {
 ...
 */
uint8_t Intel82541::in8(uint64_t address) {
 ...
 * @return the character at the given address.
 */
void Intel82541::out8(uint64_t address, uint8_t value) {
 ...
 */
uint16_t Intel82541::readPHY(uint8_t address) {
 ...
 */
void Intel82541::writePHY(uint8_t address, uint16_t value) {
