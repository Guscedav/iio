...
const int32_t PCANpci::PRIORITY = RealtimeThread::RT_MAX_PRIORITY-10;
const double PCANpci::PERIOD = 0.0001;
const uint16_t PCANpci::CHIP_OFFSET[] = {0x0000, 0x0400};
 ...
 */
PCANpci::PCANpci(PCI& pci, uint16_t number, uint16_t port) : RealtimeThread("PCANpci", STACK_SIZE, PRIORITY, PERIOD), pci(pci) {
 ...
 */
void PCANpci::frequency(uint32_t hz) {
 ...
 */
int32_t PCANpci::write(CANMessage canMessage) {
...
int32_t PCANpci::read(CANMessage& canMessage) {
 ...
 */
void PCANpci::transmit(CANMessage canMessage) {
 ...
 * @return 0 if no message was received, 1 if a message could be read successfully.
 */
int32_t PCANpci::receive(CANMessage& canMessage) {
