...
const int32_t PCANpcie::PRIORITY = RealtimeThread::RT_MAX_PRIORITY-10;
const double PCANpcie::PERIOD = 0.0005;
const uint16_t PCANpcie::CHIP_OFFSET[] = {0x0000, 0x0400};
 ...
 */
PCANpcie::PCANpcie(PCI& pci, uint16_t number, uint16_t port) : RealtimeThread("PCANpci", STACK_SIZE, PRIORITY, PERIOD), pci(pci) {
    ...
    if (baseAddress == 0) throw invalid_argument("PCANpci: this board was not found on the PCI bus.");
    if ((port != 0) && (port != 1)) throw invalid_argument("PCANpci: invalid port number, expected 0 or 1.");
 ...
 */
void PCANpcie::frequency(uint32_t hz) {
 ...
 */
int32_t PCANpcie::write(CANMessage canMessage) {
 ...
 */
int32_t PCANpcie::read(CANMessage& canMessage) {
 ...
 */
void PCANpcie::transmit(CANMessage canMessage) {
 ...
 * @return 0 if no message was received, 1 if a message could be read successfully.
 */
int32_t PCANpcie::receive(CANMessage& canMessage) {
